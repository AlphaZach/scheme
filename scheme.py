...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        return self.apply(operands.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 5
...
def complete_eval(val, env, tail=False):
    ...
    if isinstance(val, Thunk):
        ...
        return scheme_eval(val, env, tail)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # BEGIN PROBLEM 20
            "*** YOUR CODE HERE ***"
            proc = scheme_eval(first, env, True)
            check_procedure(proc)
            args = rest.map(lambda operand: (complete_eval(operand, env, True)))
            if any([isinstance(n, Thunk) for n in Pair_To_List(args)]):
                result.expr.second = args
            else:
